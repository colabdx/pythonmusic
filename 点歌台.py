...
def play_double_A_Flat_major():
    key_signature = key.KeySignature(-11)
    print(f"Key Signature: {key_signature}")

    scale = key.Key('A--').getScale('major')
...
# Create a button for double A Flat Major
play_double_A_Flat_major = tk.Button(root, text="Ases", font=('Helvetica', 24), command=play_double_A_Flat_major)
play_double_A_Flat_major.pack()
